...
Source104: nginx.conf
Source105: nginx.vh.default.conf
Source106: nginx.vh.example_ssl.conf
Source107: nginx.suse.init
Source108: nginx.service
Source109: nginx.upgrade.sh
Source110: nginx.suse.logrotate
...
%define MOD1 $(basename %SOURCE1 .tar.gz)
%define MOD2 $(basename %SOURCE2 .tar.gz)
...
./configure \
        ...
        --with-ipv6 \
        --with-debug \
        --add-module=%{_builddir}/%{MOD1} \
        --add-module=%{_builddir}/%{MOD2} \
...
%{__install} -m 644 -p %{SOURCE104} \
   $RPM_BUILD_ROOT%{_sysconfdir}/nginx/nginx.conf
%{__install} -m 644 -p %{SOURCE105} \
   $RPM_BUILD_ROOT%{_sysconfdir}/nginx/conf.d/default.conf
%{__install} -m 644 -p %{SOURCE106} \
   $RPM_BUILD_ROOT%{_sysconfdir}/nginx/conf.d/example_ssl.conf

%{__mkdir} -p $RPM_BUILD_ROOT%{_sysconfdir}/sysconfig
%{__install} -m 644 -p %{SOURCE103} \
...
%if 0%{?suse_version} == 1110
%{__install} -m755 %{SOURCE107} \
   $RPM_BUILD_ROOT%{_initrddir}/nginx
%else
%{__install} -m755 %{SOURCE102} \
   $RPM_BUILD_ROOT%{_initrddir}/nginx
%endif
%endif
